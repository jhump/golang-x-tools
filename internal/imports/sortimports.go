...
func sortImports(env *ProcessEnv, fset *token.FileSet, f *ast.File) {
	for i, d := range f.Decls {
		...
		// Merge runs of imports and then sort
		for j, s := range d.Specs {
			if j > 0 {
				for fset.Position(s.Pos()).Line > 1+fset.Position(d.Specs[j-1].End()).Line {
					fset.File(d.Rparen).MergeLine(fset.Position(d.Specs[j-1].End()).Line)
				}
			}
		}
